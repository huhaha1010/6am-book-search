...
class DoubanSpider(CrawlSpider):
    ...
    def douban_parse(self, response):
        try:
            ...
            item['book_name'] = response.xpath("//h1/span/text()").extract()[0]
            item['book_author'] = response.xpath("//div[@id='info']/a[1]/text()").extract()[0]
            item['book_score'] = response.xpath("//strong[@class='ll rating_num ']/text()").extract()[0]
            item['book_id'] = get_md5(response.url)
            item['book_source'] = "豆瓣读书"
            item['book_type'] = " "
